...
echo -e "\n=== FORGE SCRIPT DEPLOY ==="
forge script ${FEE_TOKEN_ARG[@]+"${FEE_TOKEN_ARG[@]}"} script/Mail.s.sol --private-key "$TEST_PRIVATE_KEY" --rpc-url "$TEMPO_RPC_URL" --broadcast ${VERIFY_ARGS[@]+"${VERIFY_ARGS[@]}"} --sig "run(string)" "$(date +%s%N)"
...
echo -e "\n=== FORGE CREATE DEPLOY ==="
forge create ${FEE_TOKEN_ARG[@]+"${FEE_TOKEN_ARG[@]}"} src/Mail.sol:Mail --private-key "$TEST_PRIVATE_KEY" --rpc-url "$TEMPO_RPC_URL" --broadcast ${VERIFY_ARGS[@]+"${VERIFY_ARGS[@]}"} --constructor-args 0x20c0000000000000000000000000000000000000
