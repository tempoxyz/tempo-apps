...
legacyVerifyRoute.post('/vyper', async context => {
  try {
    ...
    const { address, chain, files, contractPath, contractName, compilerVersion, compilerSettings } =
      body
    ...
    if (!Address.validate(address, { strict: true })) {
      return sourcifyError(context, 400, 'invalid_address', `Invalid address: ${address}`)
    }

    if (!files || Object.keys(files).length === 0) {
      return sourcifyError(context, 400, 'missing_files', 'No source files provided')
    }
    ...
    const container = getContainer(context.env.VERIFICATION_CONTAINER, 'singleton')
    ...
    const compiledContract = compileOutput.contracts?.[contractPath]?.[contractName]
    if (!compiledContract)
      return sourcifyError(
        context,
        400,
    ...
    const compiledBytecode = `0x${compiledContract.evm.deployedBytecode.object}` as const
    const creationBytecodeRaw = `0x${compiledContract.evm.bytecode.object}` as const
    ...
    if (runtimeMatchResult.match === null) {
      return context.json(
        {
          error: runtimeMatchResult.message ?? "The deployed and recompiled bytecode don't match."
        },
        500
      )
    }
    ...
    const runtimeCodeHashSha256 = new Uint8Array(
      await globalThis.crypto.subtle.digest('SHA-256', new TextEncoder().encode(compiledBytecode))
    )
    const runtimeCodeHashKeccak = Hex.toBytes(keccak256(compiledBytecode))

    // Compute hashes for creation bytecode
    const creationBytecode = `0x${compiledContract.evm.bytecode.object}` as const
    const creationBytecodeBytes = Hex.toBytes(creationBytecode)
    const creationCodeHashSha256 = new Uint8Array(
      await globalThis.crypto.subtle.digest('SHA-256', new TextEncoder().encode(creationBytecode))
    )
