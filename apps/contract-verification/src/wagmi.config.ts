...
const verifierUrl = import.meta.env.VITE_VERIFIER_URL ?? 'https://contracts.tempo.xyz'
...
export const tempoMainnet = {
  ...
  rpcUrls: {
    default: {
      http: [`https://proxy.tempo.xyz/rpc/4217?key=${import.meta.env.VITE_TEMPO_MAINNET_RPC_KEY}`]
    }
  }
} as const
...
export const chainIds = [tempoDevnet.id, tempoTestnet.id, tempoMainnet.id] as const
export type ChainId = (typeof chainIds)[number]
export const chains = [tempoDevnetExtended, tempoTestnetExtended, tempoMainnet] as const
...
export const getWagmiConfig = () => {
  wagmiConfigInstance ??= createConfig({
    ...
    transports: {
      ...
      [tempoTestnet.id]: fallback([http(tempoTestnet.rpcUrls.default.http.at(0))]),
      [tempoMainnet.id]: fallback([http(tempoMainnet.rpcUrls.default.http.at(0))])
    }
  })
