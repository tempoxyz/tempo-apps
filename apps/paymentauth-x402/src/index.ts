...
function createChallenge(
	...
	// Use "tempo" as the method since we're using Tempo for payments
	// The realm "x402" indicates this is an x402-compatible server
	...
	// Clean up expired challenges (simple garbage collection)
...
/**
 * GET /ping/paid - Paid endpoint (0.01 USD on Tempo via x402 protocol)
 *
 * This server implements the x402 protocol but uses Tempo for payment processing.
 * The server speaks our IETF Payment Auth protocol format, which is compatible
 * with the client.
 *
 * Flow:
 ...
 * 3. Server verifies, broadcasts to Tempo, and returns Payment-Receipt
 */
app.get('/ping/paid', async (c) => {
	...
	if (!authHeader || !authHeader.startsWith('Payment ')) {
		const challenge = createChallenge(c.env, {
			description:
				'Pay 0.01 USD to access the paid ping endpoint (x402 server)',
		})
	...
	if (!verification.valid || !verification.tx) {
		return c.json(
			new PaymentVerificationFailedError(
