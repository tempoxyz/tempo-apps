...
export type FetchAccountTransactionsArgs = {
...
export async function fetchAccountTransactions(
	args: FetchAccountTransactionsArgs,
	...
	if (includeSent) directConditions.push(`t."from" = '${args.address}'`)
	if (includeReceived) directConditions.push(`t."to" = '${args.address}'`)

	const transferConditions: string[] = []
	if (includeSent) transferConditions.push(`tr."from" = '${args.address}'`)
	if (includeReceived) transferConditions.push(`tr."to" = '${args.address}'`)
	...
	const nextOffset = args.offset + transactions.length
